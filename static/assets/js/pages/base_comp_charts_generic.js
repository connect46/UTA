...
var BaseCompCharts = function() {
    ...
    var initChartsChartJS = function () {
		//declare variables - is this needed?
		...
		//check if chart type objects exist first
		var $chartLinesList = jQuery('.js-chartjs-lines').get();
		//alert('chartlist is '+JSON.stringify($chartLinesList));
		//alert('first element is '+JSON.stringify($chartLinesList[0]));
		var $chartLinesTest  = jQuery('.js-chartjs-lines')[0];
		//alert('first element jquery is '+JSON.stringify($chartLinesTest));
		//alert('orig container jquery is '+JSON.stringify(jQuery('.js-chartjs-lines')[0].getContext('2d')));
		var $chartBarsList = jQuery('.js-chartjs-bars').get();
		...
		//process line type
		if (($chartLinesList!=null) && ($chartLinesList.length>0)){
			...
			var $chartLinesData = {
					...
					datasets: [
						{
							...
							data: [131,130,121,123,122,137,127]
						},
						{
							label: 'Diastolic',
							...
							data: [82,83,84,89,82,89,80]
						}
					]
				},
				my_line_chart_02:{ //BMI
					labels: ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL'],
					datasets: [
						{
							label: 'BMI',
							...
							pointHighlightFill: '#fff',
							pointHighlightStroke: 'rgba(220,220,220,1)',
							data: [23,24,20,22,23,25,26]
						}
					]
				},
                my_line_chart_03:{ //pulse
                    labels: ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL'],
                    datasets: [
                        {
                            label: 'Pulse',
                            ...
                            pointHighlightFill: '#fff',
                            pointHighlightStroke: 'rgba(220,220,220,1)',
                            data: [66,83,81,75,83,82,80]
                        }
                    ]
                },
                my_line_chart_04:{ //respiration
                    labels: ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL'],
                    datasets: [
                        {
                            label: 'Respiration',
                            ...
                            pointHighlightFill: '#fff',
                            pointHighlightStroke: 'rgba(220,220,220,1)',
                            data: [20,22,22,24,26,24,28]
                        }
                    ]
                },
                my_line_chart_05:{ //temperature
                    labels: ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL'],
                    datasets: [
                        {
                            label: 'Temperature',
                            ...
                            pointHighlightFill: '#fff',
                            pointHighlightStroke: 'rgba(220,220,220,1)',
                            data: [99,100,101,101,99,101,100]
                        }
                    ]
                },
                my_line_chart_06:{ //weight
                    labels: ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL'],
                    datasets: [
                        {
                            label: 'Weight',
                            ...
                            pointHighlightFill: '#fff',
                            pointHighlightStroke: 'rgba(220,220,220,1)',
                            data: [134,140,116,128,116,140]
                        }
                    ]
                },
                my_line_chart_07:{ //girth
                    labels: ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL'],
                    datasets: [
                        {
                            label: 'Girth',
                            ...
                            pointHighlightFill: '#fff',
                            pointHighlightStroke: 'rgba(220,220,220,1)',
                            data: [32, 40, 45, 43, 44, 44]
                        }
                    ]
                },
                my_line_chart_08:{ //height
                    labels: ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL'],
                    datasets: [
                        {
                            label: 'Height',
                ...
                my_line_chart_09:{ //dash.volume
                    labels: ['8/1', '8/8', '8/15', '8/22', '8/29', '9/6', '9/13'],
                    datasets: [
                        {
                            label: 'Created',
                            fillColor: 'rgba(220,220,220,.3)',
                        ...
                        },
                        {
                            ...
                            pointHighlightFill: '#fff',
                            pointHighlightStroke: 'rgba(171, 227, 125, 1)',
                            data: [14,18,14,22,18,24,25]
                        }
                    ]
                }
			};
			/*jQuery('.js-chartjs-lines').each(function() { 
					var $chartCon=$(this).getContext('2d');
					var chartid=$(this).attr("id");
					if ($chartLinesData[chartid]!=null){
						$chartLines = new Chart($chartCon).Line($chartLinesData[chartid], $globalOptions);
					} else {
						alert("id "+chartid+" for line chart not found in data");
					}
			});*/
			$chartLinesList.map(function(chart){
				...
				if (checkVar==true){
					var $chartCon=chart.getContext('2d');
					var chartid=chart.id;
					//alert('line chart id is '+chartid);
		...
		//bar charts
		if (($chartBarsList!=null) && ($chartBarsList.length>0)){
			//now set data for charts
			var $chartBarsData = {
				my_bar_chart_01:{
					labels: ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL'],
			...
			$chartBarsList.map(function(chart){
				//check if the object has any properties
				...
				if (checkVar==true){
					...
					//alert('line chart id is '+chartid);
					if ((chartid!=null) && (chartid!="")){
						if ($chartBarsData[chartid]!=null){
							$chartLines = new Chart($chartCon).Bar($chartBarsData[chartid], $globalOptions);
						} else {
							alert("id "+chartid+" for line chart not found in data");
						}
					}
    ...
    var initChartsSparkline = function(){
        ...
        jQuery('.js-slc-bar2').sparkline('html', $barOptions); // pulse

        $barOptions['barColor']         = '#faad7d';
        $barOptions['tooltipPrefix']    = '';
        $barOptions['tooltipSuffix']    = ' BreathsPM';
        jQuery('.js-slc-bar3').sparkline('html', $barOptions); //breaths
        ...
        $lineOptions['tooltipSuffix']   = 'lbs';
        jQuery('.js-slc-line1').sparkline('html', $lineOptions); //weight

        $lineOptions['lineColor']       = '#abe37d';
        $lineOptions['fillColor']       = '#abe37d';
        $lineOptions['tooltipPrefix']   = '';
        $lineOptions['tooltipSuffix']   = ' in';
        jQuery('.js-slc-line2').sparkline('html', $lineOptions); //height

        $lineOptions['lineColor']       = '#faad7d';
        $lineOptions['fillColor']       = '#faad7d';
        $lineOptions['tooltipPrefix']   = '';
        $lineOptions['tooltipSuffix']   = ' F';
        jQuery('.js-slc-line3').sparkline('html', $lineOptions); //temperature
